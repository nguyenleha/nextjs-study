...
export default getRequestConfig(async ({ requestLocale }) => {
    ...
    // Import từng namespace (login, user, ...)
    const [login, user, components] = await Promise.all([
        // Login messages
        import(`../locales/login/${locale}.json`).then((m) => m.default),
        import(`../locales/admin/user/${locale}.json`).then((m) => m.default),
        import(`../locales/admin/components/${locale}.json`).then((m) => m.default),
    ])
